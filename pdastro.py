...
def rmfile(filename, raiseError=1, gzip=False):
    ...
    if os.path.lexists(filename):
        ...
        if os.path.isfile(filename):
            if raiseError == 1:
                raise RuntimeError("Cannot remove %s" % filename)
            else:
                return 1
    if gzip and os.path.lexists(filename + ".gz"):
        ...
        if os.path.isfile(filename + ".gz"):
            if raiseError == 1:
                raise RuntimeError("Cannot remove %s" % filename + ".gz")
            else:
                return 2
    return 0
...
def radec2coord(ra, dec):
    ...
    try:
        ...
        print(f"cannot interpret: RA={ra} DEC={dec}")
        return None
...
class pdastroclass:
    ...
    def load_spacesep(
        self,
        filename,
        test4commentedheader=True,
        namesMapping=None,
        roundingMapping=None,
        hexcols=None,
        ...
        **kwargs,
        ...
        self.load(
            filename,
            na_values=na_values,
            test4commentedheader=test4commentedheader,
            namesMapping=namesMapping,
            roundingMapping=roundingMapping,
            delim_whitespace=delim_whitespace,
            hexcols=hexcols,
            auto_find_hexcols=auto_find_hexcols,
            verbose=verbose,
            **kwargs,
        )
        ...
        **kwargs,
        ...
        if test4commentedheader:
            ...
            if self.t.columns[0][0] == "#":
                self.t = self.t.rename(
                    columns={self.t.columns[0]: self.t.columns[0][1:]}
                )
    ...
    def write(
        ...
        **kwargs,
        ...
        if not (filename is None):
            if makepathFlag:
                if makepath4file(filename, raiseError=False):
                    errorstring = "could not make directory for %s" % filename
                    if raiseError:
            ...
            if os.path.isfile(filename):
                if overwrite:
                    os.remove(filename)
                    if os.path.isfile(filename):
                        errorstring = "could not clobber %s" % filename
                        if raiseError:
                            raise RuntimeError(errorstring)
                        # print(errorstring)
                        return 2
                    ...
                    if raiseError:
                        raise RuntimeError(
                            f"file {filename} already exists! use overwrite option..."
                        )

                    print(
                        f"Warning: file {filename} already exists, not deleting it, skipping! if you want to overwrite, use overwrite option!"
                    )
                    return 0
        ...
        if verbose > 1 and not (filename is None):
            print("Saving %d rows into %s" % (len(indices), filename))
        ...
        if len(indices) == 0:
            ...
            if filename is None:
                print(
                    self.t.loc[indices].to_string(
                        index=index, columns=columns, formatters=formatters, **kwargs
                    )
                )
            else:
                self.t.loc[indices].to_string(
        ...
        if not (filename is None):
            ...
            if not os.path.isfile(filename):
                errorstring = "could not save %s" % filename
                if raiseError:
                    raise RuntimeError(errorstring)
                # print(errorstring)
                return 3
    ...
    def formattable(
        ...
        if not (hexcols is None):
            for hexcol in hexcols:
                ...
                if self.t[hexcol].dtype == object and isinstance(
                    self.t.at[0, hexcol], str
                ):
        ...
        if maskval is None:
            ...
            (keep,) = np.where(
                bitmask.bitfield_to_boolean_mask(
                    self.t.loc[indices, maskcol].astype("int"),
    ...
    def ix_masked(self, maskcol, maskval=None, indices=None):
        ...
        if maskval is None:
            ...
            (keep,) = np.where(
                bitmask.bitfield_to_boolean_mask(
                    self.t.loc[indices, maskcol].astype("int"), ignore_flags=~maskval
                )
            )
        indices = indices[keep]
        return indices
    ...
    def ix_sort_by_cols(self, cols, indices=None, ascending=True):
        ...
        ix_sorted = (
            self.t.loc[indices].sort_values(cols, ascending=ascending).index.values
        )
    ...
    def fitsheader2table(
        self,
        fitsfilecolname,
        indices=None,
        requiredfitskeys=None,
        optionalfitskey=None,
        raiseError=True,
        verify="silentfix",
        skipcolname=None,
        headercol=None,
        ...
        for index in indices:
            ...
            hdu = fits.open(
                self.t.loc[index, fitsfilecolname],
                ext=ext,
                extname=extname,
            ...
            if requiredfitskeys != None:
                for fitskey in requiredfitskeys:
                    ...
                    if fitskey in header:
                        ...
                        if raiseError:
                            raise RuntimeError(
                                "fits key %s does not exist in file %s"
                                % (fitskey, self.t[fitsfilecolname][index])
                            )
                        else:
    ...
    def dateobs2mjd(
        ...
        if timeobscol != None:
            dateobslist = list(
                self.t.loc[indices, dateobscol] + "T" + self.t.loc[indices, timeobscol]
            )
        else:
    ...
    def calc_color(
        ...
        self.t[outcolor_err] = np.sqrt(
            np.square(self.t.loc[ix_good, df1]) + np.square(self.t.loc[ix_good, df2])
        )
    ...
    def radeccols_to_SkyCoord(
        ...
        if racol is not None:
            ra = np.array(self.t.loc[indices, racol])
        if deccol is not None:
            dec = np.array(self.t.loc[indices, deccol])
        ...
        if numflag:
            ...
            if racol is not None:
                ...
                if len(hexagesimal) == 1:
                    ...
                    print(
                        "Warning: it looks like there are inconsistent formats in RA column {racol}! checking line by line for sexagesimal"
                    )
                    check_line_by_line = True
                else:
                    raise RuntimeError(
                        "Something is wrong here when trying to determine if RA col {racol} is sexagesimal! "
                    )
            ...
            if check_line_by_line:
                ...
                for i in range(len(ra)):
                    coord[i] = SkyCoord(
                        ra[i], dec[i], frame=frame, unit=(raunits[i], u.deg)
                    )
            else:
    ...
    def assert_radec_cols_decimal_degrees(
        self,
        racol=None,
        deccol=None,
        outracol=None,
        ...
        (indices, coord) = self.radeccols_to_SkyCoord(
            racol=racol,
            deccol=deccol,
            indices=indices,
        ...
        if outracol is None:
            outracol = racol
        if outdeccol is None:
            outdeccol = deccol
        if outracol is not None:
            self.t.loc[indices, outracol] = coord.ra.degree
        if outdeccol is not None:
            self.t.loc[indices, outdeccol] = coord.dec.degree
        if coordcol is not None:
            self.t.loc[indices, coordcol] = coord
            # Don't write coordcol
            if not (coordcol in self.skipcols):
                self.skipcols.append(coordcol)

        return 0

    def assert_radec_cols_sexagesimal(
        self,
        racol=None,
        deccol=None,
        outracol=None,
        ...
        hmsdms = map(
            lambda x: x.split(" "),
            coord.to_string(sep=":", style="hmsdms", precision=precision),
        )
        # unzip pairs into a list of ra and a list of dec
        radeclist = list(zip(*hmsdms))

        if outracol is not None:
            self.t.loc[indices, outracol] = radeclist[0]
        if outdeccol is not None:
            self.t.loc[indices, outdeccol] = radeclist[1]
        if coordcol is not None:
            self.t.loc[indices, coordcol] = coord
            # Don't write coordcol
            if not (coordcol in self.skipcols):
    ...
    def flux2mag(
        ...
        if upperlim_Nsigma is None:
            ...
            # calculate the S/N
            self.t.loc[indices, "__tmp_SN"] = (
                self.t.loc[indices, fluxcol] / self.t.loc[indices, dfluxcol]
            )
        ...
        self.t.loc[indices_mag, magcol] = -2.5 * np.log10(
            self.t.loc[indices_mag, fluxcol].astype(np.float64)
        )
        self.t.loc[indices_mag, dmagcol] = (
            2.5
            / np.log(10.0)
            * self.t.loc[indices_mag, dfluxcol].astype(np.float64)
            / self.t.loc[indices_mag, fluxcol].astype(np.float64)
        )
        ...
        if len(indices_ul) > 0:
            ...
            if len(indices_ul_positive) > 0:
                ...
                self.t.loc[indices_ul_positive, magcol] = -2.5 * np.log10(
                    self.t.loc[indices_ul_positive, fluxcol].astype(np.float64)
                    + upperlim_Nsigma
                    * self.t.loc[indices_ul_positive, dfluxcol].astype(np.float64)
                )

            if len(indices_ul_negative) > 0:
                # If flux is negative: upper limit = Nsigma * dflux
                self.t.loc[indices_ul_negative, magcol] = -2.5 * np.log10(
                    upperlim_Nsigma
                    * self.t.loc[indices_ul_negative, dfluxcol].astype(np.float64)
                )
    ...
    def initspline(
        ...
        **kwargs,
        ...
        self.spline[ycol] = interp1d(
            self.t.loc[indices, xcol],
...
class pdastrostatsclass(pdastroclass):
    ...
    def set_statstring_format(
        ...
        self.str_format1 = (
            "mean:%s(%s) stdev:%s(%s) X2norm:%s Nchanged:%s Ngood:%s Nclip:%s"
            % (
                format_floats,
                format_floats,
                format_floats,
    ...
    def statstring(self):
        ...
        s = formatstring.format(
            self.statparams["i"],
            self.statparams["mean"],
            self.statparams["mean_err"],
            self.statparams["stdev"],
            self.statparams["stdev_err"],
            self.statparams["X2norm"],
            self.statparams["Nchanged"],
            self.statparams["Ngood"],
            self.statparams["Nclip"],
        )
        return s
    ...
    def calcaverage_errorcut(
        ...
        if not (Nsigma is None) and not (mean is None):
            ...
            (keep,) = np.where(
                np.absolute(self.t.loc[indices, datacol] - mean)
                <= Nsigma * self.t.loc[indices, noisecol]
            )
        ...
        if verbose > 3 and not (ix_good_bkp is None) and not (ix_good is None):
            print(
                "{} good data after sigma clipping, {} clipped".format(
                    len(ix_good), len(indices) - len(ix_good)
                )
            )
        ...
        if Ngood > 1:
            if medianflag:
                ...
                if verbose > 1:
                    print("median: {:f}".format(mean))
                stdev = np.sqrt(
                    1.0
                    / (Ngood - 1.0)
                    * np.sum(np.square(self.t.loc[ix_good, datacol] - mean))
                ) / self.c4(Ngood)
                ...
                c1 = np.sum(
                    1.0
                    * self.t.loc[ix_good, datacol]
                    / np.square(self.t.loc[ix_good, noisecol])
                )
            ...
            X2norm = (
                ...
                * np.sum(
                    np.square(
                        (self.t.loc[ix_good, datacol] - mean)
                        / self.t.loc[ix_good, noisecol]
                    )
                )
            )
    ...
    def calcaverage_sigmacut(
        ...
        if (percentile_cut is None) or (len(indices) <= percentile_Nmin):
            ...
            if not (Nsigma is None) and not (stdev is None) and not (mean is None):
                ix_good_bkp = copy.deepcopy(self.statparams["ix_good"])
                (keep,) = np.where(
                    np.absolute(self.t.loc[indices, datacol] - mean) <= Nsigma * stdev
                )
                ix_good = indices[keep]
                if verbose > 3:
            ...
            max_residual = np.percentile(
                np.absolute(self.t.loc[indices, datacol] - mean), percentile_cut
            )
            if verbose:
                print(
                    "%f percentile cut: max residual for cut: %f"
                    % (percentile_cut, max_residual)
                )
            ...
            if len(ix_good) < percentile_Nmin:
                print(
                    "Warning: %d<%d made it through the percentile cut, too few, taking the %d with the least residuals"
                    % (len(ix_good), percentile_Nmin, percentile_Nmin)
                )
                residuals = np.sort(self.t.loc[indices, rescol])
                max_residual = residuals[percentile_Nmin - 1]
                ix_good = self.ix_inrange(
                    rescol, None, max_residual, exclude_uplim=False
                )
                if len(ix_good) < percentile_Nmin:
                    raise RuntimeError(
                        "%d<%d in percentile cut!" % (len(ix_good), percentile_Nmin)
                    )
        ...
        if Ngood > 1:
            if medianflag:
                ...
                stdev = np.sqrt(
                    1.0
                    / (Ngood - 1.0)
                    * np.sum(np.square(self.t.loc[ix_good, datacol] - median))
                ) / self.c4(Ngood)
                mean = median
            else:
            ...
            if noisecol is None:
                X2norm = (
                    1.0
                    / (Ngood - 1.0)
                    * np.sum(np.square((self.t.loc[ix_good, datacol] - mean) / stdev))
                )
            else:
                X2norm = (
                    1.0
    ...
    def calcaverage_sigmacutloop(
        ...
        if maskcol != None:
            ...
            if verbose > 1:
                print(
                    "Keeping {:d} out of {:d}, skippin {:d} because of masking in column {} (maskval={})".format(
                        len(indices), Ntot, Ntot - len(indices), maskcol, maskval
                    )
                )
        else:
        ...
        if removeNaNs:
            colnames = [datacol]
            if not (noisecol is None):
                colnames.append(noisecol)
            if not (maskcol is None):
                colnames.append(maskcol)
            Ntot = len(indices)
            indices = self.ix_not_null(colnames, indices=indices)
            self.statparams["Nnan"] = Ntot - len(indices)
            if verbose > 1:
                print(
                    "Keeping {:d} out of {:d}, skippin {:d} because of null values in columns {:s}".format(
                        len(indices), Ntot, Ntot - len(indices), ",".join(colnames)
                    )
                )
        else:
            self.statparams["Nnan"] = 0

        while ((self.statparams["i"] < Nitmax) or (Nitmax == 0)) and (
            not self.statparams["converged"]
        ):
            # median only in first iteration and if wanted
            medianflag = (
                median_firstiteration
                and (self.statparams["i"] == 0)
                and (Nsigma != None)
            )
            ...
            if sigmacutFlag:
                errorflag = self.calcaverage_sigmacut(
                    datacol,
                    indices=indices,
                    noisecol=noisecol,
                ...
                errorflag = self.calcaverage_errorcut(
                    datacol,
                    noisecol,
                    indices=indices,
                    mean=self.statparams["mean"],
            ...
            if (
                errorflag
                or self.statparams["stdev"] == None
                or (self.statparams["stdev"] == 0.0 and sigmacutFlag)
                or self.statparams["mean"] == None
            ):
...
"""
    ...
    def intializecols4statparams(
        ...
        parammapping={},
        skipparams=[],
        addformat2defaultformatter=True,
        format4outvals="{:.3f}",
        setcol2None=True,
    ):
        outcols = []
