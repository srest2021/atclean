...
# TODO: fix to contain parameter columns
EFFICIENCY_TABLE_COLUMNS = ["sigma_kern", "peak_appmag", "peak_flux", "sigma_sim"]
...
class Simulation(ABC):
    ...
    def get_row_params(self):
        ...
        as column values.
        """
        parameters = {"name": self.name, "peak_mjd": self.peak_mjd}
        return parameters
...
class Model(Simulation):
    def __init__(
        self, filename, peak_appmag=None, name="Pre-SN Outburst Model", **kwargs
    ):
        Simulation.__init__(self, name=name, **kwargs)
        self.peak_appmag = peak_appmag
        # self.sigma = 2.8
    ...
    def __str__(self):
        return (
            f"Simulation of type {self.name} with peak app mag {self.peak_appmag:0.2f}"
        )


class TessModel(Model):
    def __init__(
        self, filename, peak_appmag=None, name="TESS Pre-SN Outburst Model", **kwargs
    ):
...
class SimDetecSupernova(AveragedSupernova):
    ...
    def set_valid_seasons_ix(self, valid_seasons=None, mjd0=None, column="MJDbin"):
        for control_index in range(self.num_controls + 1):
            self.avg_lcs[control_index].set_valid_seasons_ix(
                valid_seasons=valid_seasons, mjd0=mjd0, column=column
            )
...
class SimDetecLightCurve(AveragedLightCurve):
    ...
    def set_valid_seasons_ix(self, valid_seasons=None, column="MJDbin"):
        if valid_seasons is None:
            ...
            self.valid_seasons_ix = get_valid_ix(
                self.t, column, valid_seasons, indices=self.pre_mjd0_ix
            )
    ...
    def add_model(
        ...
        if verbose:
            print(
                f"Adding simulated model: peak MJD = {peak_mjd:0.2f} MJD; peak app mag = {peak_appmag:0.2f}; sigma = {model.sigma:0.2f} days"
            )
    ...
    def add_gaussian(
        ...
        if verbose:
            print(
                f"Adding simulated gaussian: peak MJD = {peak_mjd:0.2f} MJD; peak app mag = {gaussian.peak_appmag:0.2f}; sigma = {gaussian.sigma:0.2f} days"
            )
...
def get_key(sigma_kern, peak_appmag=None, peak_flux=None):
    if peak_appmag is None and peak_flux is None:
        raise RuntimeError(
            "ERROR: Cannot get the SimDetecTable key without peak app mag or flux."
        )
...
class SimDetecTable(pdastrostatsclass):
    def __init__(
        ...
        **kwargs,
        ...
        if peak_appmag is None and peak_flux is None:
            raise RuntimeError(
                "ERROR: Either peak app mag or flux required to construct SimDetecTable."
            )
    ...
    def get_efficiency(
        self, fom_limit, valid_seasons=None, column="sigma_sim", column_value=None
    ):
        if column_value is None:
            # get efficiency for all sigma_sims (entire table)
            column_ix = self.getindices()
        else:
            column_ix = self.ix_equal(
                column, column_value
            )  # np.where(self.t['sigma_sim'] == sigma_sim)[0]
        if len(column_ix) < 1:
...
class SimDetecTables:
    def __init__(
        self, sigma_kerns, num_iterations=None, peak_appmags=None, peak_fluxes=None
    ):
        ...
        if peak_appmags is None and peak_fluxes is None:
            raise RuntimeError(
                "ERROR: either peak app mags or peak fluxes required to construct SimDetecTables."
            )
    ...
    def setup(self, num_iterations):
        ...
        for sigma_kern in self.sigma_kerns:
            for peak_appmag in self.peak_appmags:
                ...
                self.d[key] = SimDetecTable(
                    sigma_kern, peak_appmag=peak_appmag, num_iterations=num_iterations
                )
    ...
    def get_efficiency(
        ...
        fom_limit,
        valid_seasons=None,
        column_name="sigma_sim",
        column_value=None,
    ):
        key = get_key(sigma_kern, peak_appmag=peak_appmag)
        return self.d[key].get_efficiency(
            fom_limit,
...
class EfficiencyTable(pdastrostatsclass):
    def __init__(
        ...
        **kwargs,
        ...
        if peak_appmags is None and peak_fluxes is None:
            raise RuntimeError(
                "ERROR: either peak app mags or peak fluxes required to construct an EfficiencyTable."
            )
        elif peak_appmags is None:
            peak_appmags = list(map(flux2mag, peak_fluxes))
        elif peak_fluxes is None:
    ...
    def setup(self):
        self.t = pd.DataFrame(
            columns=["sigma_kern", "peak_appmag", "peak_flux", "sigma_sim"]
        )
        ...
        for sigma_kern in self.sigma_kerns:
            ...
            df["peak_appmag"] = np.repeat(
                self.peak_appmags, len(self.sigma_sims[sigma_kern])
            )
            df["peak_flux"] = np.repeat(
                self.peak_fluxes, len(self.sigma_sims[sigma_kern])
            )
    ...
    def set_sigma_sims(self, sigma_sims):
        if isinstance(sigma_sims, List):
            ...
            if len(sigma_sims) != len(self.sigma_kerns):
                raise RuntimeError(
                    "ERROR: Each entry in sigma_kerns must have a matching list in sigma_sims"
                )
    ...
    def get_efficiencies(
        ...
        for i in range(len(self.t)):
            ...
            if verbose:
                print(
                    f"Getting efficiencies for sigma_kern {sigma_kern}, sigma_sim {sigma_sim}, peak_mag {peak_appmag}..."
                )

            for fom_limit in self.fom_limits[sigma_kern]:
                self.t.loc[i, f"pct_detec_{fom_limit:0.2f}"] = sd.get_efficiency(
                    sigma_kern,
                    peak_appmag,
                    fom_limit,
                    valid_seasons=valid_seasons,
    ...
    def get_subset(self, sigma_kern=None, fom_limit=None, sigma_sim=None):
        ...
        if not fom_limit is None:
            try:
                ...
                raise RuntimeError(
                    f"ERROR: no matching FOM limit {fom_limit}: {str(e)}"
                )
        else:
            for col in self.t.columns:
                if re.search("^pct_detec_", col):
...
def define_args(parser=None, usage=None, conflict_handler="resolve"):
    ...
    parser.add_argument(
        ...
        type=str,
        help="file name of JSON file with settings for this class",
    )
    parser.add_argument(
        "-e",
        "--efficiencies",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="only simulate events with peak MJDs within the observation season MJD ranges",
    )
    parser.add_argument(
...
class AtlasSimDetecLoop(SimDetecLoop):
    ...
    def generate_random_simulation(self, peak_appmag=None, **kwargs):
        if peak_appmag is None:
            raise RuntimeError(
                "ERROR: A peak apparent magnitude is required to generate a Gaussian simulation."
            )
    ...
    def calculate_efficiencies(self, fom_limits=None, valid_seasons=None):
        self.e: EfficiencyTable = EfficiencyTable(
            self.sigma_kerns,
            self.sigma_sims,
    ...
    def loop(self, flag=0x800000, num_iterations=50000):
        self.sd: SimDetecTables = SimDetecTables(
            sigma_kerns,
        ...
        for sigma_kern in self.sigma_kerns:
            ...
            for peak_index in range(len(self.peak_appmags)):
                ...
                print(
                    f"Commencing {num_iterations} iterations for peak app mag {peak_appmag} (peak flux {peak_flux})..."
                )
                for j in range(num_iterations):
                    ...
                    sim_lc = self.sn.avg_lcs[rand_control_index].add_gaussian(
                        gaussian, peak_mjd, flag=flag
                    )

                    # get max simulated FOM within 1 sigma of the peak MJD
                    max_fom_mjd, max_fom = self.get_max_fom(
                        sim_lc, peak_mjd=peak_mjd, sigma_sim=gaussian.sigma
                    )
...
class TessSimDetecLoop(SimDetecLoop):
    def __init__(self, output_dir: str, tables_dir: str, sigma_kerns: List):
        pass
...
if __name__ == "__main__":
    ...
    if args.obs_seasons:
        if len(config["observation_seasons"]) < 1:
            raise RuntimeError(
                "ERROR: Please fill out observation seasons in the config file before using the -m argument."
            )
    ...
    valid_control_ix = [
        i for i in range(1, num_controls + 1) if not i in config["skip_control_ix"]
    ]
    print(
    ...
    for obj in config["sim_settings"]:
        ...
        if args.efficiencies:
            if len(obj["fom_limits"]) < 1:
                raise RuntimeError(
                    f'ERROR: Efficiency calculation set to {args.efficiencies}, but no FOM limits provided for sigma_kern={obj["sigma_kern"]}.'
                )
    ...
    if args.efficiencies:
        simdetec.calculate_efficiencies(
            fom_limits=fom_limits, valid_seasons=valid_seasons
        )
