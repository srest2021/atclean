...
class ControlCoordinatesTable:
    ...
    def read(self, filename: str):
        ...
        with pd.option_context("display.float_format", "{:,.8f}".format):
            print(
                "Control light curve coordinates read from file: \n", self.t.to_string()
            )
    ...
    def construct_row(
        ...
        ra_offset = Angle(
            ra_distance.degree * (1.0 / math.cos(center_coords.dec.angle.radian)),
            u.degree,
        )
        ...
        if (
            ...
            if offset_sep < self.closebright_min_dist:
                print(
                    f'Control light curve {i:3d} too close to SN location ({offset_sep}" away) with minimum distance to SN as {self.closebright_min_dist}"; skipping control light curve...'
                )
                return

        self.add_row(
            np.nan, i, coords, ra_offset=ra_offset, dec_offset=dec_offset, radius=r
        )
    ...
    def construct(
        ...
        for i in range(1, self.num_controls + 1):
            self.construct_row(
                i, full_sn_lc.coords, center_coords, r, closebright=closebright
            )
...
class DownloadLoop:
    def __init__(self, args):
        ...
        if len(self.tnsnames) > 1 and (
            ...
            or not args.ctrl_coords is None
        ...
        print(
            f"Sigma limit when converting flux to magnitude : {self.flux2mag_sigmalimit}"
        )
        ...
        print()
        sninfo_filename = (
            args.sninfo_file if args.sninfo_file else config["dir"]["sninfo_filename"]
        )
        ...
        if self.controls:
            if args.ctrl_coords:
                ...
                if args.closebright:
                    self.ctrl_coords.closebright_min_dist = float(
                        config["download"]["closebright_min_dist"]
                    )
                    print(
                        f"Circle pattern centered around close bright object at {args.closebright}; minimum distance from SN set to {self.ctrl_coords.closebright_min_dist}"
                    )
                self.ctrl_coords.radius = (
                    args.radius if args.radius else float(config["download"]["radius"])
                )
                self.ctrl_coords.num_controls = (
                    args.num_controls
                    if args.num_controls
                    else int(config["download"]["num_controls"])
                )
                print(
                    f"Setting circle pattern of {self.ctrl_coords.num_controls:d} control light curves",
                    end="",
                )
                if args.closebright:
